...
module DW_BN_ReLU#(
    ...
    output [int_bits*2-1:0] reuse_g, BN_config_out_g,
    output [int_bits-1:0] out
    );

wire [int_bits-1:0] BN_config_in[1:0];
assign {BN_config_in[1],BN_config_in[0]} = BN_config_in_g;
wire [int_bits-1:0] BN_config_out[1:0];
assign BN_config_out_g = {BN_config_out[1],BN_config_out[0]};
...
assign reuse_g = {ST_ele_out[1],ST_ele_out[0]};
...
BN#( .int_bits(int_bits) ) BN(
    ...
    .in(adder_out), .config_in_g({BN_config_in[1],BN_config_in[0]}),
    .out( BN_out ), .config_out_g({BN_config_out[1],BN_config_out[0]})
);
